...
export const shoppingRoutes = async (app: FastifyInstance) => {
  app.get("/demo", async () => listMockShoppingItems());

  app.get("/demo/:id", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get("/page-cursor", async (request: FastifyRequest) => {
    const query = z.object({
  ...
  app.patch("/:id/increment", async (request: FastifyRequest) => {
    const { id } = paramsSchema.parse(request.params);
    const body = z.object({
