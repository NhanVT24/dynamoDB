type ApiGatewayAuthorizerClaims = {
...
function readGroupsFromApiGatewayEvent(encodedEvent: string | undefined) {
  if (!encodedEvent) return [];

  try {
    const decodedEvent = decodeURIComponent(encodedEvent);
    const event = JSON.parse(decodedEvent) as ApiGatewayEventLike;
...
function readGroupsFromAuthorizationHeader(authorizationHeader: string | undefined) {
  if (!authorizationHeader?.startsWith("Bearer ")) return [];

  const token = authorizationHeader.slice("Bearer ".length).trim();
  if (!token) return [];

  const payload = decodeJwtPayload<ApiGatewayAuthorizerClaims>(token);
  return toGroups(payload?.["cognito:groups"]);
}

export function extractCognitoGroups(headers: Record<string, unknown>) {
...
export function isAdminRequest(headers: Record<string, unknown>) {
