...
function buildCategoryStatusSearch(name, stock) {
    return `${deriveStatus(stock)}#${normalizeText(name ?? "")}`;
}
function toProductRecord(input, current) {
    ...
    return {
        ...
        categoryStatusSearch: input.categoryStatusSearch ?? buildCategoryStatusSearch(name, stock),
...
async function scanUntilEnough(limit, cursor, filters = {}) {
    const results = [];
    let state = cursor ? decodeCursor(cursor) : { lastKey: null };
...
function buildCategoryStatusNameQuery(filters = {}) {
    ...
    const values = { ":categoryKey": filters.category };
    const names = {};
    let filterExpression;
    if (filters.status) {
        names["#categoryStatusSearch"] = "categoryStatusSearch";
        values[":categoryStatusPrefix"] = search
            ? `${filters.status}#${search}`
            : `${filters.status}#`;
        expressionParts.push("begins_with(#categoryStatusSearch, :categoryStatusPrefix)");
    }
    else if (search) {
...
export async function incrementItemValue(id, field, incrementBy = 1) {
    ...
    const result = await rawDb.send(new UpdateItemCommand({
        ...
        UpdateExpression: [
            ...
            "#status = :status",
            "#searchName = :searchName",
            "#categoryStatusSearch = :categoryStatusSearch",
            "updatedAt = :updatedAt",
            "#version = #version + :one"
        ].join(", "),
        ...
        ExpressionAttributeValues: toDynamoItem({
            ...
            ":categoryStatusSearch": nextRecord.categoryStatusSearch,
...
export async function getCursorForPage(page = 1, limit = 12, filters = {}) {
    if (filters.sortBy) {
        ...
        const cursorHistory = Array.from({ length: resolvedPage }, (_, index) => index === 0 ? null : encodeCursor({ offset: index * pageSize }));
        return {
            page: resolvedPage,
...
export async function updateShoppingItem(id, patch, version) {
    ...
    const merged = toProductRecord({ ...patch, updatedAt: new Date().toISOString() }, current);
    const names = {
        "#version": "version",
        "#status": "status",
        "#searchName": "searchName",
    ...
    const values = {
        ...
        ":categoryStatusSearch": merged.categoryStatusSearch,
